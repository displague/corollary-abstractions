...
`scripts/story.py`, `scripts/harness.py`, plus the W1/W2 modules if
wired), and the registered run revalidates them before timing anything.
...
- **W2 — `reachable <world-id> <target-path>`** → `closure_query.query`
  ...
  (`scripts/closure_query.py:269-276`). The committed target set MUST
  ...
  Requires a new optional boot probe `closure.worlds` (OK/OFF, joining
  the six probes at `scripts/harness.py:516-523`). The route is ordered
