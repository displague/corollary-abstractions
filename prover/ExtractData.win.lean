/-
================================================================================
ExtractData.win.lean -- Windows-native fork of LeanDojo-v2's tactic-state tracer
================================================================================

ORIGIN
  Upstream file : lean_dojo_v2/lean_dojo/data_extraction/ExtractData.lean
  Package       : lean-dojo-v2 1.0.9 (PyPI wheel
                  lean_dojo_v2-1.0.9-py3-none-any.whl, obtained with
                  `pip download lean-dojo-v2 --no-deps` and unzipped; the same
                  file is on GitHub at
                  https://github.com/lean-dojo/LeanDojo/blob/main/src/lean_dojo/data_extraction/ExtractData.lean)
  Pristine sha256: b50bebdef8adde06d1f6948edc0ac52f67dd8b40045df72290a1f225049ffc50
  Vendored      : 2026-08-07, corollary-abstractions prover phase 1
  Toolchain     : requires Lean >= ~4.32 (uses String.trimAscii and a
                  MonadState instance absent in 4.20); validated on
                  leanprover/lean4:v4.32.2, x86_64-w64-windows-gnu.

WHY THIS FORK EXISTS
  The upstream tracer panics on native Windows before emitting anything:

      PANIC at LeanDojo.Path.findLean ExtractData:274:2: assertion violation
      uncaught exception: (`Inhabited.default` for `IO.Error`)

  `Path.findLean` recovers a module's source path from its `.olean` path by
  string-stripping the build directory, but the patterns it strips are written
  with hardcoded FORWARD slashes:

      let lean := olean.toString.replace ".lake/build/lib/lean/" ""
        |>.replace "build/lib/lean/" "" |>.replace "lib/lean/Lake/" ...

  On Windows `findOLean` returns backslash-separated paths
  (`C:\...\.lake\build\lib\lean\Foo.olean`), so none of the replacements
  fire, the derived `.lean` path does not exist, and `assert! <- path.pathExists`
  aborts the process.

THE PATCH (2 lines, both inside `LeanDojo.Path.findLean`)
  1. Normalize separators on the olean path before the strip chain:
       -   let lean := olean.toString.replace ".lake/build/lib/lean/" ""
       +   let lean := olean.toString.replace "\\" "/" |>.replace ".lake/build/lib/lean/" ""
  2. Normalize the cwd the derived path is made relative to, so the
     `relativeTo` component comparison sees matching separators:
       -   let cwd <- IO.currentDir
       +   let cwd := FilePath.mk ((<- IO.currentDir).toString.replace "\\" "/")

  Nothing else is changed. Line numbers below this header are shifted by the
  length of this comment relative to upstream's numbering.

USAGE (native Windows, from the target lake project's root)
  ...
  Copy-Item <repo>\prover\ExtractData.win.lean .\ExtractData.lean
  lake env lean --threads 8 --run ExtractData.lean noDeps

  Output: .lake/build/ir/<Module>.ast.json  (keys: tactics, premises, commandASTs)
          .lake/build/ir/<Module>.dep_paths

  See prover/FEASIBILITY.md for the full Windows landmine list.
-/
